...
import { JobFormSchema, showAlert, formatPhoneNo } from "./FormSupport"
import { z } from "zod"
	...
	async function onSubmit(data: FormValues) {
		...
		if (fileSizeValidator(file) && fileExtensionValidator(file.name)) {
			try {
				...
				showAlert({
					text: " there was an error sending your application",
					status: "ERR",
				})
			} finally {
			...
			if (fileSizeValidator(file) && !fileExtensionValidator(file.name)) {
				setFileError(
					"Invalid file Type. Accepted Formats: '.pdf', '.doc', '.docx', '.word', '.rtf'"
				)
			} else if (
				!fileSizeValidator(file) &&
				fileExtensionValidator(file.name)
			) {
				setFileError(
					"Max file size exceeded. File uploads must be less that 10MB."
				)
			} else if (
				!fileSizeValidator(file) &&
				!fileExtensionValidator(file.name)
	...
	return (
		<Card className="w-full h-full">
			<CardHeader className="relative flex flex-col items-center w-full py-12">
				...
				<CardDescription className="flex pt-8 pr-20 ml-auto text-2xl text-right">
			...
			<CardContent>
				<div className="bg-white isolate lg:pb-32">
					...
					<form
						...
						className="w-4/5 max-w-2xl mx-auto scrollbar-hide sm:mt-10">
							...
							<div>
								<div className="flex justify-between">
									<label
										...
										<span className="mr-1.5 text-lg font-bold text-red-600">
											*
										</span>
										Name
									</label>
						...
						<div className="flex flex-col p-3 rounded-lg sm:col-span-2 bg-zinc-300/30">
							...
							<p className="flex w-full max-w-md mx-auto text-sm text-left text-zinc-500 pr-auto">
								Please select a file to upload and select the
								corresponding purpose of the file.
							</p>
