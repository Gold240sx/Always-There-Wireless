...
type StepperFormProps = { Steps: Step[]; goTo?: (index: number) => void }
...
const StepperGraphic = ({ Steps }: StepperFormProps) => {
	...
	return (
		<nav
			aria-label="Progress"
			className="items-center hidden pb-12 mx-auto align-middle sm:flex justify-evenly">
					...
					<li key={Step.name} className="md:flex-1">
						{Step.status === "complete" ? (
							<button
								...
								<span className="text-sm font-medium pointer-events-none whitespace-nowrap text-zinc-700">
									{Step.name}
								</span>
							</button>
						) : Step.status === "current" ? (
