...
export const ApplicationFormSchema = z
	.object({
		...
		DOB: z
			...
			.refine(
				...
				{
					message:
						"Invalid date of birth. Month should be between 01 and 12, and day should be between 01 and 30.",
				}
			),
		//  .refine((value) => {
	...
	.superRefine((values, context) => {
		...
		if (
			values.address.docDifDelivAdd === "true" &&
		...
		if (
			values.address.docDifDelivAdd === "true" &&
			!values.address.physical!.state?.label
		) {
			context.addIssue({
				code: z.ZodIssueCode.custom,
